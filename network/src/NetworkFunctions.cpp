...
namespace Network
{
    ...
    void appendNetworkInformation(Initialization::ProgramPlan & plan, const NetworkPlan & netPlan)
    {
        ...
        for (const auto & toExtend : netPlan.fmuNet)
        {
            ...
            shared_ptr<Initialization::ConnectionPlan> networkToReal = nullptr;  // networkFmu -> dependent Fmu
            if (toExtend.inputMap.size() > 0)
            {
                networkToReal = std::make_shared<Initialization::ConnectionPlan>();
            ...
            shared_ptr<Initialization::ConnectionPlan> realToNetwork = nullptr;  // output/dependent fmu -> networkFmu
            if (toExtend.outputMap.size() > 0)
            {
                realToNetwork = std::make_shared<Initialization::ConnectionPlan>();
