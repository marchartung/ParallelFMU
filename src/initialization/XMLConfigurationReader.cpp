...
namespace Initialization
{
    ...
    FmuPlan XMLConfigurationReader::getFmuPlan(const ptree::value_type & fmuElem, const SimulationPlan & simPlan, const size_type id)
    {
        ...
        if (res.version != "1.0")
            throw std::runtime_error("XMLConfigurationReader: FMUs with a version higher aren't supported yet");
    ...
    SolverPlan XMLConfigurationReader::getSolverPlan(const ptree::value_type &firstElem, const SimulationPlan & simPlan, size_type id)
    {
        ...
        res.id = id;
        res.kind = firstElem.second.get<std::string>("<xmlattr>.solver", res.kind);
    ...
    std::list<SolverPlan> XMLConfigurationReader::getSolverPlanFromMultipleFmu(const ptree::value_type &firstElem, const size_type & startId,
                                                                               const SimulationPlan & simPlan)
    {
        std::list<SolverPlan> res;
    ...
    std::list<SolverPlan> XMLConfigurationReader::getSolverPlanFromFmu(const ptree::value_type &firstElem, const size_type & id, const SimulationPlan & simPlan,
                                                                       const bool & single)
    {
        SolverPlan solv = getSolverPlan(firstElem, simPlan, id);
        solv.fmu = std::shared_ptr<FmuPlan>(new FmuPlan(getFmuPlan(firstElem, simPlan, (single) ? std::numeric_limits<size_type>::max() : id)));
        ...
        if (DefaultValues::isUndefinedValue(solv.fmu->id))
            solv.fmu->id = id;
        return std::list<SolverPlan>(1, solv);
    }

    std::list<SolverPlan> XMLConfigurationReader::getSolverPlans(const SimulationPlan & simPlan)
    {
        std::list<SolverPlan> res;
        ...
        size_type idCount = 0;
        std::list<SolverPlan> tmp;
        for (ptree::value_type & firstElem : solverElem.get_child(""))
        {
            if (firstElem.first == std::string("fmu"))
                tmp = getSolverPlanFromFmu(firstElem, idCount, simPlan, true);
            else if (firstElem.first == std::string("multipleFmu"))
                tmp = getSolverPlanFromMultipleFmu(firstElem, idCount, simPlan);
            else
                throw std::runtime_error("XMLConfigurationReader: Unknown fmu type.");
            idCount += tmp.size();
    ...
    ConnectionPlan XMLConfigurationReader::getConnectionPlan(ptree::value_type & mapElem)
    {
        ...
        {
            std::cout << "XMLConfigurationReader: Unkown connection type \"" << mapElem.first << "\"";
            return res;
        }
        ...
        for (ptree::value_type &varMapElem : mapElem.second.get_child(""))
        {
            ...
            if (varType == "real")
                ...
                throw std::runtime_error("XMLConfigurationReader: unkown variable type " + varType);
        }
        checkForUndefinedValues(res.bufferSize, res.destFmu, res.sourceFmu, res.inputMapping);
        return res;
    }

    std::list<ConnectionPlan> XMLConfigurationReader::getConnectionPlans()
    {
        std::list<ConnectionPlan> res;
    ...
    std::vector<std::vector<size_type>> XMLConfigurationReader::getNodeSchedule(ptree::value_type & nodeElem)
    {
        ...
        for (ptree::value_type &coreElem : nodeElem.second.get_child(""))
        {
            ...
            {
                res.push_back(std::vector<size_type>(coreElem.second.get<size_type>("<xmlattr>.numFmus", 0)));
            }
            else if (coreElem.first == "cores")
            {
                res.resize(res.size() + coreElem.second.get<size_type>("<xmlattr>.numCores", 0),
                           std::vector<size_type>(coreElem.second.get<size_type>("<xmlattr>.numFmusPerCore", 0), 0));
            }
        }
        return res;
    }
            ...
            if (!schedElem.empty())
            {
                for (ptree::value_type &nodeElem : schedElem.get_child(""))
                {
                    ...
                    else if (nodeElem.first == "nodes")
                    {
                        ...
                        if (numNewNodes == 0)
                            throw std::runtime_error("In tag <nodes> the attribute 'numNodes' is missing.");
                        size_type numNewCores = nodeElem.second.get<size_type>("<xmlattr>.numCoresPerNode", 0);
                        if (numNewCores == 0)
                            throw std::runtime_error("In tag <nodes> the attribute 'numCoresPerNode' is missing.");
                        size_type numNewFmus = nodeElem.second.get<size_type>("<xmlattr>.numFmusPerCore", 0);
                        if (numNewFmus == 0)
                            throw std::runtime_error("In tag <nodes> the attribute 'numFmusPerCore' is missing.");
                        res.nodeStructure.resize(res.nodeStructure.size() + numNewNodes,
                                                 std::vector<std::vector<size_type> >(numNewCores, std::vector<size_type>(numNewFmus)));
                    }
                    else
                        throw std::runtime_error("XMLReader: In scheduling was an unknown node identifier.");
                }
            }
        }
        return res;
                                                             ...
                                                             const SchedulePlan & schedPlan)
    {
        ...
        {
            tmp = std::shared_ptr<ConnectionPlan>(new ConnectionPlan(cp));
    ...
    WriterPlan XMLConfigurationReader::getWriterPlan()
    {
        ...
        if (res.filePath == "")
            throw std::runtime_error("XMLConfigurationReader: Result file path not set");
        //TODO read vars which you want to write

        return res;
    }

    void XMLConfigurationReader::createMapping(const std::list<SolverPlan> & solverPlans, SchedulePlan & schedPlan)
    {
    ...
    ProgramPlan XMLConfigurationReader::getProgramPlan()
    {
        ...
        std::list<SolverPlan> solverPlans = getSolverPlans(simPlan);
        std::list<ConnectionPlan> connPlans = getConnectionPlans();

        SchedulePlan schedPlan = getSchedulePlan();
        if (schedPlan.nodeStructure.empty())
        {
            //TODO do scheduling
            schedPlan.nodeStructure = std::vector<std::vector<std::vector<size_type>>>(1,std::vector<std::vector<size_type>>(1,std::vector<size_type>(1)));
            schedPlan.solvIdToCore.resize(solverPlans.size());
