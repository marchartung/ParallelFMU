...
static char* getTmpPath() {
  char temp[13];  // Lenght of "fmuTmpXXXXXX" + null
  sprintf(temp, "%s", "fmuTmpXXXXXX");
  //char *tmp = mkdtemp(strdup("fmuTmpXXXXXX"));
  char *tmp = mkdtemp(temp);
  if (tmp==NULL) {
...
FMU* loadFMU(const char* fmuFileName) {
    ...
    fmu->path = getFmuPath(fmuFileName);
    if (!fmu->path) exit(EXIT_FAILURE);

    // unzip the FMU to the tmpPath directory
    tmpPath = getTmpPath();
    if (!unzip(fmu->path, tmpPath)) exit(EXIT_FAILURE);
    ...
    // load the FMU dll
    dllPath = (char*)calloc(sizeof(char), strlen(tmpPath) + strlen(DLL_DIR)
    ...
    if (!loadDll(dllPath, fmu)) {
        ...
        free(dllPath);
        dllPath = (char*)calloc(sizeof(char), strlen(tmpPath) + strlen(DLL_DIR2)
...
{
/*    deleteUnzippedFiles(fmu); TODO*/
    if(fmu->dllHandle != 0)
        dlclose(fmu->dllHandle);
    free(fmu);
}

void deleteUnzippedFiles(FMU *fmu) {
    char *fmuTempPath = fmu->tmpFolder;
