...
const char *elmNames[SIZEOF_ELM] = { "fmiModelDescription", "UnitDefinitions", "BaseUnit", "DisplayUnitDefinition", "TypeDefinitions", "Type", "RealType",
        "IntegerType", "BooleanType", "StringType", "EnumerationType", "Item", "DefaultExperiment", "VendorAnnotations", "Tool", "Annotation", "ModelVariables",
        "ScalarVariable", "DirectDependency", "Name", "Real", "Integer", "Boolean", "String", "Enumeration", "Implementation", "CoSimulation_StandAlone",
        "CoSimulation_Tool", "Model", "File", "Capabilities" };

const char *attNames[SIZEOF_ATT] = { "fmiVersion", "displayUnit", "gain", "offset", "unit", "name", "description", "quantity", "relativeQuantity", "min", "max",
        "nominal", "declaredType", "start", "fixed", "startTime", "stopTime", "tolerance", "value", "valueReference", "variability", "causality", "alias",
        "modelName", "modelIdentifier", "guid", "author", "version", "generationTool", "generationDateAndTime", "variableNamingConvention",
        "numberOfContinuousStates", "numberOfEventIndicators", "input", "canHandleVariableCommunicationStepSize", "canHandleEvents", "canRejectSteps",
        "canInterpolateInputs", "maxOutputDerivativeOrder", "canRunAsynchronuously", "canSignalEvents", "canBeInstantiatedOnlyOncePerProcess",
        "canNotUseMemoryManagementFunctions", "file", "entryPoint", "manualStart", "type" };

const char *enuNames[SIZEOF_ENU] = { "flat", "structured", "constant", "parameter", "discrete", "continuous", "input", "output", "internal", "none", "noAlias",
        "alias", "negatedAlias" };
...
// enuNames[id] is the string_type representation of the enum value.
        ...
        *vs = valueMissing;
        switch (a)
        {
            ...
            case att_alias:
                return enu_noAlias;
            default:
                return enu_BAD_DEFINED;
        }
    }
    id = checkEnumValue(value);
    if (id == enu_BAD_DEFINED)
        *vs = valueIllegal;
    return id;
}
...
double getRealStartValue(ScalarVariable *modelVariable, ValueStatus* vs)
{
    Element* typeElem = modelVariable->typeSpec;
    assert(checkElementType(typeElem, elm_Real));
    return getDouble(typeElem, att_start, vs);
}

int getIntStartValue(ScalarVariable *modelVariable, ValueStatus* vs)
{
    Element* typeElem = modelVariable->typeSpec;
    assert(checkElementType(typeElem, elm_Integer));
    return getInt(typeElem, att_start, vs);
}

int getEnumStartValue(ScalarVariable *modelVariable, ValueStatus* vs)
{
    Element* typeElem = modelVariable->typeSpec;
    assert(checkElementType(typeElem, elm_Enumeration));
    return getInt(typeElem, att_start, vs);
}

char getBoolStartValue(ScalarVariable *modelVariable, ValueStatus* vs)
{
    Element* typeElem = modelVariable->typeSpec;
    assert(checkElementType(typeElem, elm_Boolean));
    return getBoolean(typeElem, att_start, vs);
}

const char* getStringStartValue(ScalarVariable *modelVariable, ValueStatus* vs)
{
    Element* typeElem = modelVariable->typeSpec;
    assert(checkElementType(typeElem, elm_String));
    const char * res = getString(typeElem, att_start);
    if (res != NULL)
        *vs = ValueStatus::valueDefined;
    else
        *vs = ValueStatus::valueMissing;
    return res;
}

double getDefaultStopTime(ModelDescription* md)
{
    ValueStatus vs;
    assert(md->defaultExperiment);
    double n = getDouble(md->defaultExperiment, att_stopTime, &vs);
    assert(vs == valueDefined);  // this is a required attribute
    return n;
}

double getDefaultStartTime(ModelDescription* md)
{
    ValueStatus vs;
    assert(md->defaultExperiment);
    double n = getDouble(md->defaultExperiment, att_startTime, &vs);
    assert(vs == valueDefined);  // this is a required attribute
    return n;
}
...
VarType getVariableType(ScalarVariable *modelVariable)
{
    Element* typeElem = modelVariable->typeSpec;
    int typecheck = checkElementType(typeElem, elm_Real);
    if (typecheck)
        return varReal;
    typecheck = checkElementType(typeElem, elm_Integer);
    if (typecheck)
        return varInt;
    typecheck = checkElementType(typeElem, elm_Boolean);
    if (typecheck)
        return varBool;
    typecheck = checkElementType(typeElem, elm_Enumeration);
    if (typecheck)
        return varEnum;
    typecheck = checkElementType(typeElem, elm_String);
    assert(typecheck);
    return varString;
}

VarCausality getVariableCausality(ScalarVariable *modelVariable)
{
    Enu causality = getCausality(modelVariable);
    if (causality == enu_input)
        return varCausalityInput;
    if (causality == enu_output)
        return varCausalityOutput;
    if (causality == enu_internal)
        return varCausalityInternal;
    assert(causality == enu_none);
    return varCausalityNone;
}
...
Type* getDeclaredType(ModelDescription* md, const char* declaredType)
{
    ...
    if (declaredType && md->typeDefinitions)
        for (i = 0; md->typeDefinitions[i]; i++)
        {
            ...
            if (!strcmp(declaredType, getName(tp)))
                return tp;
        }
    return NULL;
}
...
{
    logThis(ERROR_FATAL, "Wrong element type, expected %s, found %s", expected, elmNames[found]);
    XML_StopParser(parser, XML_FALSE);
}

// Returns 0 to indicate error
// Verify that Element elm is of the given type
int checkElementType(void* element, Elm e)
{
...
static void XMLCALL endElement(void *context, const char *elm)
{
    ...
    switch (el)
    {
        case elm_fmiModelDescription:
        {
            ...
            if (!checkElementType(child, elm_fmiModelDescription))
                return;
        ...
        case elm_ScalarVariable:
        {
            ...
            if (!checkPeek(elm_ScalarVariable))
                return;
...
// Feature in expat:
// For some reason, if the element data is the empty string_type (Eg. <a></a>)
// instead of an empty string_type with len == 0 we get "\n". The workaround is
// to replace this with the empty string_type whenever we encounter "\n".
...
ModelDescription* parse(const char* xmlPath)
{
    ...
    if (!checkPointer(stack))
        return NULL;  // failure
    parser = XML_ParserCreate(NULL);
    if (!checkPointer(parser))
        return NULL;  // failure
